...
public class TileEntityBarrel extends TileEntity implements IInventory {
	...
	public String getInventoryName() {
		return this.hasCustomInventoryName() ? this.localizedName : "container.barrel";
	}
	...
	private static int getItemBurnTime(ItemStack itemstack) {
		if(itemstack == null){
			...
			if(item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.air) {
				...
				if(item == MicranyftMod.hop) return 100;
				if(item == MicranyftMod.grapes) return 100;
			}
	...
	public void updateEntity() {
		...
		if(!this.worldObj.isRemote) {
			...
			if(flag != this.isBurning()) {
				flag1 = true;
				BlockBarrel.updateBlockBarrelState(this.burnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
		if(flag1) {
			this.markDirty();
